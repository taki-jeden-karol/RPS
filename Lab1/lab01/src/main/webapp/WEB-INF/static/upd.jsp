...
<c:url value="/master/upd/${master.id}" var="crtMastasUrl" />
<form id="addTaskForm" action="${crtMastasUrl}" method="POST">
    <table border="4" style="font-family:Comfortaa,serif ;">
        <tr>
            <td><label>Имя:</label></td>
            <td><input name="name" type="text"
                       value="<c:out value="${master.name}"/>"/>
            </td>
        </tr>
        <tr>
            <td><label>Адрес:</label></td>
            <td><input name="address" type="text"
                       value="<c:out value="${master.address}"/>"/>
            </td>
        </tr>
        <tr>
            <td><label>Имя питомца:</label></td>
            <td><input name="petname" type="text"
                       value="<c:out value="${master.petname}"/>"/>
            </td>
        </tr>
        <tfoot>
